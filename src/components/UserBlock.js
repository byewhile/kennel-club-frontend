...
export default function UserBlock({ user, setUserId, setAction, setIsShowConfirmBlock }) {
    return (
        <div className="flex flex-col lg:flex-row items-center justify-between gap-5 shadow rounded p-5">
            ...
            <div className="flex lg:justify-end w-full lg:w-32">
                <div className="flex mt-3 font-medium">
                    ...
                    {user.role !== "admin" && (
                        <>
                            <button onClick={() => {setUserId(user.id); setAction("make_admin"); setIsShowConfirmBlock(true)}} className="hover:text-yellow-400 focus:text-yellow-400 text-gray-300 p-2 rounded cursor-pointer transition" title="Сделать администратором">
                                <FaCrown />
                            </button>

                            <button onClick={() => {setUserId(user.id); setAction("delete"); setIsShowConfirmBlock(true)}} className="hover:text-red-500 focus:text-red-500 text-gray-300 p-2 rounded cursor-pointer transition" title="Удалить пользователя">
                                <FaTrash />
                            </button>
                        </>
                    )}
